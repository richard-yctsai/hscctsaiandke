...
/**
* This class is used to calculate similarity score with a certain fusion algorithm.
*
* @author  WeiChun
*/
public class FusionAlgo {
	...
	*   
	* @param: turn recorded form skeleton data
	* @param: turn recorded form inertial data
	* 
	...
	*/
	public static double calResult_alg1(TurnList fromVideo, TurnList fromIMU) {
		...
		//System.out.println(mixedturnsQueue_vMiS.size() + " - " + mixedturnsQueue_vSiM.size());
		...
		//System.out.println("Result: " + result + " Cnt: " + resultCnt + " Score: " + result / resultCnt);
	...
	*/
	public static double calResult_alg2(TurnList fromVideo, TurnList fromIMU) {
		...
		double[] xcorr = calXCorr(KinectAccSeq[0], IMUAccSeq[0], 60);
		double max = 0;
	...
	*/
	public static double calResult_alg3(TurnList fromVideo, TurnList fromIMU) {
		...
		/*for (int i = 0; i < xcorr[0].length; i++) {
			System.out.println(xcorr[0][i]);
		}*/
		
		/*for (int i = 0; i < KinectAccSeq[0].length; i++) {
			System.out.printf("%f ", KinectAccSeq[2][i]);
		}
		System.out.println("");
		
		for (int i = 0; i < IMUAccSeq[0].length; i++) {
			System.out.printf("%f ", IMUAccSeq[3][i]);
		}
		System.out.println("");*/
		...
		/*for (int i = 0; i < 3; i++) {
			double tmpXCorr = -1;
			for (int j = 0; j < 2 * lag + 1; j++) {
				tmpXCorr = (tmpXCorr > xcorr[i][j])?tmpXCorr:xcorr[i][j];
			}
			max += tmpXCorr;
		}*/
	...
	/**
	* Generate queue of "Stop" turns from skeleton data and inertial data.
	* The list will be used to calculate S-S overlaps
	*   
	* @param: turn recorded form skeleton data
	* @param: turn recorded form inertial data
	* 
	...
	*/
	...
	/**
	* Generate queue of "Move" turns from skeleton data and inertial data.
	* The list will be used to calculate M-M overlaps
	*   
	* @param: turn recorded form skeleton data
	* @param: turn recorded form inertial data
	* 
	...
	*/
	...
	/**
	* Generate mixed queue of "Move" turns from skeleton data "Stop" turns from inertial data.
	* The list will be used to calculate M-S overlaps
	*   
	* @param: turn recorded form skeleton data
	* @param: turn recorded form inertial data
	* 
	...
	*/
	...
	/**
	* Generate mixed queue of "Stop" turns from skeleton data "Move" turns from inertial data.
	* The list will be used to calculate S-M overlaps
	*   
	* @param: turn recorded form skeleton data
	* @param: turn recorded form inertial data
	* 
	...
	*/
	private static double[] calBonus(ArrayList<STurn> stillturnsQueue, ArrayList<MTurn> moveturnsQueue) {
		...
		for (int i = 0; i < stillturnsQueue.size() - 1; i++) {
			double overlap = Math.min(stillturnsQueue.get(i).getEndTime() - stillturnsQueue.get(i+1).getStartTime(), stillturnsQueue.get(i+1).getEndTime() - stillturnsQueue.get(i+1).getStartTime());
		...
		for (int i = 0; i < moveturnsQueue.size() - 1; i++) {
			double overlap = Math.min(moveturnsQueue.get(i).getEndTime() - moveturnsQueue.get(i+1).getStartTime(), moveturnsQueue.get(i+1).getEndTime() - moveturnsQueue.get(i+1).getStartTime());
			overlap = new BigDecimal(overlap).setScale(1, BigDecimal.ROUND_HALF_DOWN).doubleValue();
			if (overlap >= 0) {
	...
	private static double[] calPenalty(ArrayList<Turn> mixedturnsQueue_vMiS, ArrayList<Turn> mixedturnsQueue_vSiM) {
		...
		for (int i = 0; i < mixedturnsQueue_vMiS.size() - 1; i++) {
			double overlap = Math.min(mixedturnsQueue_vMiS.get(i).getEndTime() - mixedturnsQueue_vMiS.get(i+1).getStartTime(), mixedturnsQueue_vMiS.get(i+1).getEndTime() - mixedturnsQueue_vMiS.get(i+1).getStartTime());
			overlap = new BigDecimal(overlap).setScale(1, BigDecimal.ROUND_HALF_DOWN).doubleValue();
			if (overlap > 0) {
				penalty += bf.getPenaltyValue(overlap);
				//System.out.println("Penalty: " + overlap + " - " + bf.getPenaltyValue(overlap));
				penaltyCnt += 1;
			}
		}
		
		for (int i = 0; i < mixedturnsQueue_vSiM.size() - 1; i++) {
			double overlap = Math.min(mixedturnsQueue_vSiM.get(i).getEndTime() - mixedturnsQueue_vSiM.get(i+1).getStartTime(), mixedturnsQueue_vSiM.get(i+1).getEndTime() - mixedturnsQueue_vSiM.get(i+1).getStartTime());
			overlap = new BigDecimal(overlap).setScale(1, BigDecimal.ROUND_HALF_DOWN).doubleValue();
			if (overlap > 0) {
				penalty += bf.getPenaltyValue(overlap);
				//System.out.println("Penalty: " + overlap + " - " + bf.getPenaltyValue(overlap));
	...
	private static double[][] getAccSeq(TurnList turnsList) {
		...
		if (MTurns != null && STurns == null) {
			...
			for (int k = 0; k < STurns.size(); k++) {
				double dur = new BigDecimal(STurns.get(i).getEndTime() - STurns.get(i).getStartTime()).setScale(1, BigDecimal.ROUND_HALF_DOWN).doubleValue();
			...
			while (true) {
				if (i < STurns.size() && j < MTurns.size() && STurns.get(i).getStartTime() < MTurns.get(j).getStartTime()) {
					double dur = new BigDecimal(STurns.get(i).getEndTime() - STurns.get(i).getStartTime()).setScale(1, BigDecimal.ROUND_HALF_DOWN).doubleValue();
					time = new BigDecimal(dur/0.1).setScale(0, BigDecimal.ROUND_HALF_DOWN).intValue();
					for (int t = 0; t < time * 2; t++) {
						acc.add(0.0);
					...
					i += 1;
				} else if (i < STurns.size() && j < MTurns.size() && MTurns.get(j).getStartTime() < STurns.get(i).getStartTime()) {
					for (Double stepAcc:MTurns.get(j).getAcc()) {
						acc.add(stepAcc);
					}
					for (Double stepAcc:MTurns.get(j).getAcc_x()) {
	...
	/*private static double calPCC(double[] kAccSeq, double[] iAccSeq) {
		double num, den, den1, den2;
		double sum_x = 0, sum_y = 0, sum_xx = 0, sum_xy = 0, sum_yy = 0;
		int len = (kAccSeq.length < iAccSeq.length)? kAccSeq.length : iAccSeq.length;
		
		for (int i = 0; i < len; i++) {
			sum_xy += kAccSeq[i] * iAccSeq[i];
			sum_x += kAccSeq[i];
			sum_y += iAccSeq[i];
			sum_xx += kAccSeq[i] * kAccSeq[i];
			sum_yy += iAccSeq[i] * iAccSeq[i];
		}
		
		num = (len * sum_xy) - (sum_x * sum_y);
		den1 = (len * sum_xx) - (sum_x * sum_x);
		den2 = (len * sum_yy) - (sum_y * sum_y);
